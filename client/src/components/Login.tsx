...
import { Search, X, LogIn, Check, ImageIcon, XCircle } from 'lucide-react';
...
export default function Login() {
 ...
 setBusy(false);
  ...
  const track = async () => {
  const q = lrr.trim().toUpperCase();
  if (!q) return;
  setTrackBusy(true);
  setTrackError('');
  setTrackResult(null);
  try {
  const res = await fetch(`${BASE}/track/${encodeURIComponent(q)}`);
  if (!res.ok) {
 ...
 <div className="flex flex-col items-center mb-8">
  <svg viewBox="0 0 468.62 308.07" className="w-28 h-auto mb-4 shrink-0 overflow-visible" xmlns="http://www.w3.org/2000/svg" stroke="white" strokeWidth="4">
 ...
 <path fill="#2c2660" d="M316.71,195.75c56.09,3.79,132.62-1.53,81.87-52.57,167.77,86.27-32.39,103.03-112.46,94.04,21.81,5.54,43.38,10.16,65.81,13.18-46.91,57.98-139.65,72.74-200.41,28.35-4.62-1.27-10.26-5.81-13.24-9.9-20-14.53-35.64-37.53-44.43-60.51-7.13-11.58-3.13-30.9-18.12-35.66-21.19-11.73-44.12-24.77-59.48-43.55-36.66-45.51,42.57-55.41,73.7-53.75-57.67,14.58-9.19,57.7,22.06,70.16-19.92,15.22,50.4,24.85,61.44,28.05,0,.19.01.39.02.58-22.96,0-43.87-11.79-65.73-17.49-31.12,19.93,49.3,35.4,64.28,36.02,16.68,33.06,53.53-4.46,32.18-29.03,14.92,2.92,2.69,22.52,7.53,28.68,10.98-1.64,61.52,7.78,61.19-2.06,8.68,1.93,25.11,4.21,43.8,5.48"/>
  ...
  <h1 className="text-2xl font-bold text-white ">Planet Transport Pvt Ltd.</h1>
  <p className="text-base text-white/80">Tengpora Bypass, Srinagar</p>
 </div>

 <div className="bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
 {/* Track Consignment */}
 <div className="p-5">
 <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Track Consignment</h2>
 <div className="flex items-center gap-2 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 focus-within:ring-2 focus-within:ring-gray-400 :ring-gray-500">
 <Search size={16} className="text-gray-400 shrink-0" />
 <input value={lrr} onChange={(e) => setLrr(e.target.value.toUpperCase())}
 onKeyDown={(e) => e.key === 'Enter' && track()}
 placeholder="Enter LRR number"
 className="flex-1 bg-transparent text-sm outline-none text-gray-800 placeholder:text-gray-400" />
 {lrr && <button onClick={() => { setLrr(''); setTrackResult(null); setTrackError(''); }} className="p-0.5 hover:bg-gray-200 :bg-gray-700 rounded"><X size={14} className="text-gray-400" /></button>}
 <button onClick={track} disabled={trackBusy || !lrr.trim()}
 className="bg-gray-900 text-white px-3 py-1.5 rounded text-xs font-medium hover:bg-gray-800 :bg-gray-200 disabled:opacity-50 shrink-0">
 {trackBusy ? '...' : 'Track'}
 </button>
 </div>

 {trackError && <p className="mt-3 text-xs text-red-600 bg-red-50 px-3 py-2 rounded-lg">{trackError}</p>}
  ...
  <p className="text-sm font-bold ">{trackResult.booking.booking_no}</p>
  </div>
  ...
  <div><span className="text-gray-400">From:</span> <span className="">{trackResult.booking.from_location || trackResult.booking.pickup_location || '-'}</span></div>
  <div><span className="text-gray-400">To:</span> <span className="">{trackResult.booking.to_location || trackResult.booking.delivery_location || '-'}</span></div>
  </div>
    ...
    {trackResult.stages.map((stage: any, i: number) => {
      ...
      return (
        <div key={i} className="flex items-start gap-3 pb-4 last:pb-0 relative">
          <div className={`w-[22px] h-[22px] rounded-full flex items-center justify-center shrink-0 z-10 ${
            isDone ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-400'
          }`}>
            {isDone ? <Check size={12} strokeWidth={3} /> : <div className="w-2 h-2 rounded-full bg-gray-300" />}
          </div>
          <div className="text-xs leading-5">
            <span className={`font-medium ${isDone ? 'text-gray-900' : 'text-gray-400'}`}>
              {stageLabels[stage.icon] || stage.label}
            </span>
            {stage.warehouse && <span className="text-gray-400 ml-1">- {stage.warehouse}</span>}
            {stage.date && <span className="text-gray-400 ml-2">{stage.date}</span>}
          </div>
        </div>
      );
    })}
  ...
  {trackResult.pod_photo && (
    <button onClick={() => setPhotoModal(trackResult.pod_photo)}
      className="flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-medium mt-2">
      <ImageIcon size={14} /> View POD Photo
    </button>
  )}
  </div>
  )}
 ...
 {/* Employee Login */}
 <div className="p-5">
 <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-3">Employee Login</h2>
 <form onSubmit={handleSubmit} className="space-y-3">
 {error && <div className="bg-red-50 text-red-600 text-xs px-3 py-2 rounded-lg border border-red-200 ">{error}</div>}
  <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-400 :border-gray-500 bg-gray-50 text-gray-900 placeholder:text-gray-400"
  placeholder="Username" required autoComplete="username" autoCapitalize="none" style={{ textTransform: 'none' }} />
 <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
 className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-gray-400 :border-gray-500 bg-gray-50 text-gray-900 placeholder:text-gray-400"
 placeholder="Password" required autoComplete="current-password" />
 <button type="submit" disabled={busy}
 className="w-full bg-gray-900 text-white py-2 rounded-lg hover:bg-gray-800 :bg-gray-200 text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
 <LogIn size={15} /> {busy ? 'Logging in...' : 'Log in'}
  ...
  <p className="text-center text-[10px] text-gray-400 mt-4">Planet Transport Pvt Ltd &mdash; Logistics ERP</p>
  </div>
