...
const DELIVERY_TABS = ['send', 'delivered', 'dlrs'] as const;
type DeliveryTab = (typeof DELIVERY_TABS)[number];

export default function Deliveries({ tabs, actions }: { tabs?: string[] | null; actions?: string[] | null }) {
  ...
  const allowedTabs = tabs ? DELIVERY_TABS.filter((t) => tabs.includes(t)) : [...DELIVERY_TABS];
  const defaultTab = allowedTabs[0] || 'send';
  ...
  const [tab, setTab] = useState<DeliveryTab>(defaultTab);
  ...
  const [deliveredLRs, setDeliveredLRs] = useState<any[]>([]);
  const [loadingDelivered, setLoadingDelivered] = useState(false);
  const [dlrsSearch, setDlrsSearch] = useState('');
 ...
 <h1 className="text-2xl font-bold text-gray-800 ">Delivery</h1>
  ...
  {canCreate && (
    <button onClick={() => { setDpEdit(null); setDpForm({ name: '', phone: '', vehicle_number: '' }); setDpModal(true); }} className="flex items-center gap-1.5 bg-white border border-gray-200 text-gray-600 px-4 py-2 rounded-lg hover:bg-gray-50 :bg-gray-800 text-sm font-medium">
   ...
   {allowedTabs.includes('send') && (
     <button onClick={() => setTab('send')} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${tab === 'send' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>Send for Delivery</button>
   )}
   {allowedTabs.includes('delivered') && (
 ...
 <h2 className="font-semibold text-gray-700 mb-4">Send for Delivery</h2>
 <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
 <div>
 <label className="block text-xs font-medium text-gray-500 mb-1">Delivery Person</label>
 <select value={selectedDP} onChange={(e) => setSelectedDP(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 ...
 {deliveryPersons.filter((p) => p.status !== 'Inactive').map((dp) => <option key={dp.id} value={dp.id!}>{dp.name}{dp.vehicle_number ? ` (${dp.vehicle_number})` : ''}</option>)}
  ...
  <input type="date" value={deliveryDate} onChange={(e) => setDeliveryDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
  </div>
  ...
  <label className="block text-xs font-medium text-gray-500 ">Select LR(s) for Delivery ({selectedLRs.length} selected)</label>
  <input type="text" placeholder="Search LR number..." value={lrSearch} onChange={(e) => setLrSearch(e.target.value)} className="w-48 border rounded px-2 py-1 text-xs focus:ring-2 focus:ring-indigo-500 outline-none" />
  </div>
  {availableLRs.length === 0 ? (
  <p className="text-sm text-gray-400 py-4 text-center border rounded-lg bg-gray-50 ">No loaded LRs available. Load a vehicle first.</p>
  ) : (
 ...
 className={`flex items-center gap-3 px-3 py-2 text-sm cursor-pointer border-b last:border-0 ${selected ? 'bg-indigo-50 border-indigo-200' : 'hover:bg-gray-50 :bg-gray-800 '}`}
 >
  ...
  <span className="text-xs text-gray-600 w-36 truncate">{b.consignor_name || '-'}</span>
  <span className="text-xs text-gray-600 w-36 truncate">{b.consignee_name || '-'}</span>
  <span className="text-xs text-gray-400 w-24">{b.from_location || '-'} → {b.to_location || '-'}</span>
  <span className="text-xs text-gray-500 w-20">{b.actual_weight ? `${b.actual_weight} kg` : '-'}</span>
  <span className="text-xs text-gray-400 w-28">{b.eway_bill_no || '-'}</span>
 </div>
 );
 })}
 </div>
   ...
   <h2 className="font-semibold text-gray-700 mb-4">Confirm Delivery</h2>
   <div className="flex items-end gap-3 mb-4">
   <div className="flex-1">
   <label className="block text-xs font-medium text-gray-500 mb-1">Delivery Number</label>
   <input type="text" placeholder="Type delivery number like DLV-..." value={confirmDeliveryNo} onChange={(e) => setConfirmDeliveryNo(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && searchDelivery()} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
   </div>
   ...
   {confirmDelivery && (
   <div>
   <div className="bg-gray-50 rounded-lg p-3 mb-3 text-xs text-gray-600 flex gap-4 flex-wrap">
   ...
   <label className="block text-xs font-medium text-gray-500 mb-2">Select LRs that were delivered (unchecked LRs will go back for re-delivery)</label>
   ...
   <tr key={b.id} onClick={() => toggleConfirmLR(b.id)} className={`cursor-pointer text-xs hover:bg-gray-50 :bg-gray-800 ${confirmSelected.has(b.id) ? 'bg-green-50 ' : 'bg-red-50/50 '}`}>
   ...
   <button onClick={() => { setConfirmDelivery(null); setConfirmDeliveryNo(''); }} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 :bg-gray-800 rounded-lg">Cancel</button>
   ...
   <input type="text" placeholder="Search by LR number..." value={dlrsSearch} onChange={(e) => setDlrsSearch(e.target.value)} className="w-full max-w-xs border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
   </div>
   <div className="overflow-x-auto">
   {loadingDelivered ? (
   <div className="text-center py-10 text-gray-400 text-sm">Loading...</div>
   ) : deliveredLRs.length === 0 ? (
   <div className="text-center py-10 text-gray-400 text-sm">No delivered LRs yet</div>
   ) : (
   <table className="w-full text-sm">
   <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
   <tr>
   ...
   <th className="text-left py-3 px-4">Nags</th>
   <th className="text-left py-3 px-4">Delivery #</th>
   <th className="text-left py-3 px-4">Delivery Person</th>
   <th className="text-left py-3 px-4">Delivered At</th>
   <th className="text-left py-3 px-4">POD</th>
   </tr>
   </thead>
   <tbody className="divide-y divide-gray-50">
    {deliveredLRs.filter((lr) => !dlrsSearch || lr.booking_no?.toLowerCase().includes(dlrsSearch.toLowerCase())).map((lr) => (
   <tr key={lr.id} className="hover:bg-gray-50 text-xs">
   <td className="py-3 px-4 font-mono font-medium">{lr.booking_no}</td>
   <td className="py-3 px-4">{lr.lr_date ? new Date(lr.lr_date).toLocaleDateString() : '-'}</td>
   <td className="py-3 px-4 max-w-[120px] truncate" title={lr.consignor_name}>{lr.consignor_name || '-'}</td>
   <td className="py-3 px-4 max-w-[120px] truncate" title={lr.consignee_name}>{lr.consignee_name || '-'}</td>
   <td className="py-3 px-4">{lr.from_location || '-'} → {lr.to_location || '-'}</td>
   <td className="py-3 px-4">{lr.num_bags ?? '-'}</td>
   <td className="py-3 px-4 font-mono">{lr.delivery_no || '-'}</td>
   <td className="py-3 px-4">{lr.delivery_person_name || '-'}</td>
   <td className="py-3 px-4">{lr.delivered_at ? new Date(lr.delivered_at).toLocaleString() : '-'}</td>
   <td className="py-3 px-4">{lr.pod_photo ? <button onClick={() => setViewPhoto(lr.pod_photo)} className="text-indigo-600 hover:text-indigo-800 flex items-center gap-1"><Camera size={13} /> View</button> : '-'}</td>
   </tr>
   ))}
   </tbody>
     </table>
 ...
 <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-xs font-medium">{d.status}</span>
 </td>
 <td className="py-3 px-4">
 ...
 <div><span className="text-gray-400 ">Date:</span> {detail.delivery_date ? new Date(detail.delivery_date).toLocaleDateString() : '-'}</div>
 <div><span className="text-gray-400 ">Delivery Person:</span> {detail.delivery_person_name || '-'} {detail.delivery_person_phone ? `| ${detail.delivery_person_phone}` : ''} {detail.delivery_person_vehicle ? `| ${detail.delivery_person_vehicle}` : ''}</div>
 </div>
 ...
 <th className="text-left py-2 px-3">Nags</th>
              ...
              <td className="py-2 px-3">{b.pod_photo ? <button onClick={() => setViewPhoto(b.pod_photo)} className="text-indigo-600 hover:text-indigo-800 text-[11px] flex items-center gap-1"><Camera size={12} /> View</button> : '-'}</td>
              </tr>
              ))}
              </tbody>
 ...
 <div className="space-y-4">
 <div className="text-xs text-gray-400 ">{deliveryPersons.length} person(s)</div>
 <div className="border rounded-lg">
 <div className="overflow-x-auto">
 <table className="w-full text-sm">
 <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
 ...
 <tr key={p.id} className="hover:bg-gray-50 :bg-gray-800 text-xs">
 <td className="py-2 px-3 font-medium">{p.name}</td>
 <td className="py-2 px-3">{p.phone || '-'}</td>
 <td className="py-2 px-3">{p.vehicle_number || '-'}</td>
 <td className="py-2 px-3">
 <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-medium ${p.status === 'Active' ? 'bg-green-100 text-green-700 ' : 'bg-gray-100 text-gray-600'}`}>{p.status || 'Active'}</span>
 </td>
 <td className="py-2 px-3 text-right">
  <div className="flex gap-1 justify-end">
  {canEdit && <button onClick={() => { setDpEdit(p); setDpForm({ name: p.name, phone: p.phone || '', vehicle_number: p.vehicle_number || '' }); }} className="p-1 hover:bg-gray-100 :bg-gray-800 rounded"><Edit2 size={12} className="text-gray-400" /></button>}
  {canDelete && <button onClick={() => removeDP(p.id!)} className="p-1 hover:bg-red-50 rounded"><X size={12} className="text-red-400" /></button>}
  </div>
 </td>
 </tr>
 ))}
 ...
 <h4 className="text-xs font-semibold text-gray-600 mb-3">{dpEdit ? 'Edit Delivery Person' : 'Add Delivery Person'}</h4>
 <div className="grid grid-cols-3 gap-3">
 <div>
 <label className="block text-xs font-medium text-gray-500 mb-1">Name *</label>
 <input value={dpForm.name} onChange={(e) => setDpForm({ ...dpForm, name: e.target.value })} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
 </div>
 <div>
 <label className="block text-xs font-medium text-gray-500 mb-1">Phone</label>
 <input value={dpForm.phone || ''} onChange={(e) => setDpForm({ ...dpForm, phone: e.target.value })} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
 </div>
 <div>
 <label className="block text-xs font-medium text-gray-500 mb-1">Vehicle</label>
 <input value={dpForm.vehicle_number || ''} onChange={(e) => setDpForm({ ...dpForm, vehicle_number: e.target.value })} className="w-full border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
 </div>
 </div>
