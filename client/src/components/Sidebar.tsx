...
export default function Sidebar({ current, onNavigate, mobileOpen, onMobileClose, allowedModules }: {
  ...
  const filteredItems = allowedModules
    ? menuItems.filter((m) => allowedModules.has(m.id))
    : menuItems;
  ...
  const sidebarContent = (
    <>
      <div className="flex items-center justify-between px-4 h-14 shrink-0">
        {!collapsed && (
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-semibold text-base tracking-tight">PlanetERP</span>
          </div>
        )}
        <div className="flex items-center gap-1">
          {collapsed && (
            <button onClick={() => setCollapsed(false)} className="p-1.5 rounded-lg hover:bg-white/10 text-gray-400 hover:text-white transition-colors">
              <ChevronRight size={16} />
            </button>
          )}
          <button onClick={onMobileClose} className="p-1.5 rounded-lg hover:bg-white/10 text-gray-400 md:hidden transition-colors">
            <X size={16} />
          </button>
        </div>
      </div>
      ...
      <div className="px-3 mb-1">
        {!collapsed && (
          <button onClick={() => setCollapsed(true)} className="flex items-center gap-2 w-full px-2 py-1.5 text-xs text-gray-500 hover:text-gray-300 transition-colors rounded-md hover:bg-white/5">
            <ChevronLeft size={14} />
            <span>Collapse</span>
          </button>
        )}
      ...
      <div className="border-t border-white/10 px-3 py-3 flex flex-col gap-1">
        <button
          onClick={toggle}
          className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-400 hover:text-white hover:bg-white/5 rounded-lg transition-all"
        ...
        <button
          onClick={() => { logout(); }}
          className="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-gray-400 hover:text-red-400 hover:bg-white/5 rounded-lg transition-all"
          title="Sign Out"
        >
          <LogOut size={18} />
  ...
  return (
    <>
      ...
      <aside className={`fixed md:hidden z-50 inset-y-0 left-0 bg-[#1a1a2e] text-white flex flex-col transition-all duration-200 ease-in-out ${
        mobileOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        {sidebarContent}
      </aside>
      <aside className={`hidden md:flex ${collapsed ? 'w-16' : 'w-60'} bg-[#1a1a2e] text-white flex-col transition-all duration-200 ease-in-out shrink-0`}>
        {sidebarContent}
      </aside>
    </>
  );
