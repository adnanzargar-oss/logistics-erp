import {
  ...
  Building2, Warehouse, PackageCheck, Package,
...
export default function Sidebar({ current, onNavigate, mobileOpen, onMobileClose, allowedModules }: {
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-700">
        {!collapsed && (
          <div className="flex items-center gap-2">
            <svg viewBox="0 0 468.62 308.07" className="w-6 h-4 shrink-0" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className="font-bold text-lg tracking-tight">PlanetERP</span>
          </div>
        )}
        <button onClick={() => setCollapsed(!collapsed)} className="p-1 rounded hover:bg-slate-700 hidden md:block">
          {collapsed ? <ChevronRight size={18} /> : <ChevronLeft size={18} />}
        </button>
        <button onClick={onMobileClose} className="p-1 rounded hover:bg-slate-700 md:hidden">
          <X size={18} />
        </button>
      </div>
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm transition-colors ${
                active ? 'bg-indigo-600 text-white border-r-2 border-indigo-300' : 'text-slate-300 hover:bg-slate-800 hover:text-white'
              }`}
  ...
  return (
    <>
      ...
      {/* Mobile drawer */}
      <aside className={`fixed md:hidden z-50 inset-y-0 left-0 bg-slate-900 text-white flex flex-col transition-all duration-200 ease-in-out ${
        mobileOpen ? 'translate-x-0' : '-translate-x-full'
      }`}>
        {sidebarContent}
      </aside>
      {/* Desktop sidebar */}
      <aside className={`hidden md:flex ${collapsed ? 'w-16' : 'w-60'} bg-slate-900 text-white flex-col transition-all duration-200 ease-in-out shrink-0`}>
        {sidebarContent}
      </aside>
    </>
  );
