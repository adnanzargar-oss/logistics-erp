    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Fleet Maintenance</h1>
        </div>
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-6">
          ...
          <div className="flex gap-3 overflow-x-auto scrollbar-hide">
            {upcoming.slice(0, 5).map((m) => (
              <div key={m.id} className="bg-white dark:bg-gray-900 rounded-lg px-3 py-2 text-xs border border-amber-100 whitespace-nowrap">
              ...
              {records.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900">
                  ...
                  <td className="py-3 px-4">
                    <button onClick={() => remove(r.id!)} className="p-1 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"><Trash2 size={14} className="text-red-400" /></button>
                  </td>
                </tr>
              ))}
              {records.length === 0 && <tr><td colSpan={10} className="text-center py-10 text-gray-400 dark:text-gray-500">No maintenance records yet</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Add Service Record" wide>
        <div className="grid grid-cols-2 gap-3">
          ...
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Service Date *</label><input type="date" value={form.service_date} onChange={(e) => setForm({ ...form, service_date: e.target.value })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Service Type *</label>
            <select value={form.service_type} onChange={(e) => setForm({ ...form, service_type: e.target.value })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
          ...
          <div className="col-span-2"><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Description</label><textarea value={form.description || ''} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" rows={2} /></div>
          ...
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Provider Name</label><input value={form.provider_name || ''} onChange={(e) => setForm({ ...form, provider_name: e.target.value })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Labor Cost</label><input type="number" value={form.labor_cost || ''} onChange={(e) => { const v = Number(e.target.value); setForm({ ...form, labor_cost: v, total_cost: calcTotal(v, form.parts_cost || 0, form.tax_amount || 0) }); }} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Parts Cost</label><input type="number" value={form.parts_cost || ''} onChange={(e) => { const v = Number(e.target.value); setForm({ ...form, parts_cost: v, total_cost: calcTotal(form.labor_cost || 0, v, form.tax_amount || 0) }); }} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Tax Amount</label><input type="number" value={form.tax_amount || ''} onChange={(e) => { const v = Number(e.target.value); setForm({ ...form, tax_amount: v, total_cost: calcTotal(form.labor_cost || 0, form.parts_cost || 0, v) }); }} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          ...
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Next Service Date</label><input type="date" value={form.next_service_date || ''} onChange={(e) => setForm({ ...form, next_service_date: e.target.value })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
          <div><label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1">Next Service (km)</label><input type="number" value={form.next_service_km || ''} onChange={(e) => setForm({ ...form, next_service_km: Number(e.target.value) || undefined })} className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
        </div>
