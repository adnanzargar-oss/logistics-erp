  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800 dark:text-white">Nag Barcode Printing</h1>
      </div>
      ...
      {loading ? (
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((b) => (
                <tr key={b.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-750">
                  <td className="p-3 font-medium text-gray-800 dark:text-white">{b.booking_no}</td>
                  <td className="p-3 text-gray-600 dark:text-gray-300">{b.consignor_name || '-'}</td>
                  <td className="p-3 text-gray-600 dark:text-gray-300">{b.consignee_name || '-'}</td>
                  <td className="p-3 text-gray-600 dark:text-gray-300">{(b.from_location || '-')} → {(b.to_location || '-')}</td>
                  <td className="p-3 text-center font-bold text-blue-600 dark:text-blue-400">{b.num_bags}</td>
