  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    cells.push(
      <div
        ...
        className={`p-1 min-h-[70px] border border-gray-200 dark:border-gray-700 rounded cursor-pointer transition-colors ${
          isToday ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-300 dark:border-indigo-700' : 'hover:bg-gray-50 dark:hover:bg-gray-800'
        }`}
      >
        <div className={`text-xs font-semibold mb-0.5 ${isToday ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'}`}>{d}</div>
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-xl font-bold text-gray-800 dark:text-white">Operations Calendar</h1>
      ...
      <div className="flex items-center justify-between mb-3">
        <button onClick={prevMonth} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300">
          <ChevronLeft size={20} />
        </button>
        <span className="text-lg font-bold text-gray-800 dark:text-white">{MONTHS[month]} {year}</span>
        <button onClick={nextMonth} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300">
        ...
        {(['loading', 'receiving', 'delivery'] as const).map(t => (
          <button
            ...
            className={`rounded-lg p-3 transition-all ${
              filters[t]
                ? `${typeColor(t).replace('bg-', 'bg-').replace('-500', '-50')} dark:${typeColor(t).replace('bg-', 'bg-').replace('-500', '-900/20')}`
          ...
          >
            ...
            <div className={`text-xs ${filters[t] ? 'text-gray-600 dark:text-gray-300' : 'text-gray-400'}`}>
      ...
      {selectedDay && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setSelectedDay(null)}>
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[80vh] overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
              ...
              <div className="flex items-center gap-1">
                <button onClick={() => printDay(selectedDay)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded text-gray-400 hover:text-indigo-600">
                  <Printer size={16} />
                </button>
                <button onClick={() => setSelectedDay(null)} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded">
                ...
                return (
                  <div key={t}>
                    ...
                    <div className="space-y-2">
                      {group.map((evt, i) => (
                        <div key={i} className="flex items-start gap-3 p-3 rounded-lg border border-gray-200 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-700/30">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between">
                              <span className="text-sm font-semibold text-gray-800 dark:text-white">{evt.ref}</span>
