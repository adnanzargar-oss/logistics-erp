...
export default function Invoices() {
 ...
 return (
 <tr key={inv.id} className="hover:bg-gray-50 :bg-gray-800 ">
 ...
 <button onClick={() => setDetail(inv)} className="p-1 hover:bg-gray-100 :bg-gray-800 rounded"><Eye size={14} className="text-gray-400 " /></button>
 <button onClick={() => remove(inv.id!)} className="p-1 hover:bg-red-50 :bg-red-900/20 rounded"><Trash2 size={14} className="text-red-400" /></button>
 </div>
 </td>
 </tr>
 );
 })}
 {invoices.length === 0 && <tr><td colSpan={9} className="text-center py-10 text-gray-400 ">No invoices yet</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 <select value={form.customer_id || ''} onChange={(e) => setForm({ ...form, customer_id: Number(e.target.value) })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 <option value="">Select customer</option>
 {customers.map((c) => <option key={c.id} value={c.id!}>{c.name} {c.company ? `(${c.company})` : ''}</option>)}
 </select>
 </div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Invoice Date *</label><input type="date" value={form.invoice_date} onChange={(e) => setForm({ ...form, invoice_date: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Due Date</label><input type="date" value={form.due_date || ''} onChange={(e) => setForm({ ...form, due_date: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Subtotal *</label><input type="number" value={form.subtotal || ''} onChange={(e) => { const s = Number(e.target.value); const t = calcTax(s, form.tax_percent || 0); setForm({ ...form, subtotal: s, ...t }); }} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Tax (%)</label><input type="number" value={form.tax_percent || ''} onChange={(e) => { const p = Number(e.target.value); const t = calcTax(form.subtotal || 0, p); setForm({ ...form, tax_percent: p, ...t }); }} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Tax Amount</label><input type="number" value={form.tax_amount || 0} className="w-full border rounded-lg px-3 py-2 text-sm bg-gray-50 text-gray-600 " readOnly /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Total Amount</label><input type="number" value={form.total_amount || 0} className="w-full border rounded-lg px-3 py-2 text-sm bg-gray-50 font-bold" readOnly /></div>
 ...
 <div className="grid grid-cols-2 gap-3">
 <div><span className="text-gray-400 ">Customer:</span> <span className="font-medium">{detail.customer_name}</span></div>
 <div><span className="text-gray-400 ">Company:</span> {detail.customer_company || '-'}</div>
 <div><span className="text-gray-400 ">Invoice Date:</span> {new Date(detail.invoice_date).toLocaleDateString()}</div>
 <div><span className="text-gray-400 ">Due Date:</span> {detail.due_date ? new Date(detail.due_date).toLocaleDateString() : '-'}</div>
 </div>
 <hr />
 <div className="space-y-1">
 <div className="flex justify-between"><span className="text-gray-400 ">Subtotal:</span><span>{(detail.subtotal || detail.total_amount || 0).toLocaleString()}</span></div>
 <div className="flex justify-between"><span className="text-gray-400 ">Tax ({(detail.tax_percent || 0)}%):</span><span>{(detail.tax_amount || 0).toLocaleString()}</span></div>
 ...
 <div className="flex justify-between"><span className="text-gray-400 ">Paid:</span><span className="text-green-600">{(detail.paid_amount || 0).toLocaleString()}</span></div>
 <div className="flex justify-between"><span className="text-gray-400 ">Balance:</span><span className="text-red-600">{((detail.total_amount || 0) - (detail.paid_amount || 0)).toLocaleString()}</span></div>
 </div>
