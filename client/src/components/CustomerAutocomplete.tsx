...
export default function CustomerAutocomplete({ customers, value, onChange, onSelect, placeholder }: Props) {
 ...
 className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none pr-8"
 />
 ...
 className="px-3 py-2 text-sm hover:bg-indigo-50 :bg-indigo-900/30 cursor-pointer border-b last:border-0"
 >
 <div className="font-medium">{c.name}</div>
 <div className="text-[10px] text-gray-400 ">{c.company || ''} {c.gstin ? `GST: ${c.gstin}` : ''} {c.phone ? `| ${c.phone}` : ''}</div>
 </div>
 ))}
 </div>
 )}
