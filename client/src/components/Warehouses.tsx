...
import { api } from '../api';
import { Warehouse, WarehouseCamera } from '../types';
import { Plus, Edit2, Trash2, Printer, Warehouse as WarehouseIcon, Camera, X, Monitor } from 'lucide-react';
...
export default function Warehouses() {
  ...
  const [cameras, setCameras] = useState<WarehouseCamera[]>([]);
  const [cameraWarehouse, setCameraWarehouse] = useState<Warehouse | null>(null);
  const [cameraManageOpen, setCameraManageOpen] = useState(false);
  const [viewCameras, setViewCameras] = useState<WarehouseCamera[]>([]);
  const [viewWarehouse, setViewWarehouse] = useState<Warehouse | null>(null);
  const [camForm, setCamForm] = useState({ name: '', url: '' });
  const [editingCam, setEditingCam] = useState<WarehouseCamera | null>(null);
  ...
  const isStreamUrl = (url: string) => /\.(jpg|jpeg|png|mjpg)$/i.test(url) || /stream/i.test(url);

  const loadCameras = async (wh: Warehouse) => {
    const list = await api.cameras.list(wh.id!);
    setCameras(list);
    setCameraWarehouse(wh);
    setCameraManageOpen(true);
  };

  const saveCam = async () => {
    if (!cameraWarehouse?.id || !camForm.name || !camForm.url) return;
    if (editingCam?.id) {
      await api.cameras.update(editingCam.id, camForm);
    } else {
      await api.cameras.create(cameraWarehouse.id, camForm);
    }
    setCamForm({ name: '', url: '' });
    setEditingCam(null);
    const list = await api.cameras.list(cameraWarehouse.id);
    setCameras(list);
  };
  ...
  const editCam = (c: WarehouseCamera) => {
    setEditingCam(c);
    setCamForm({ name: c.name, url: c.url });
  };

  const deleteCam = async (id: number) => {
    if (!confirm('Delete this camera?')) return;
    await api.cameras.delete(id);
    if (cameraWarehouse) loadCameras(cameraWarehouse);
  };

  const openViewer = async (wh: Warehouse) => {
    const list = await api.cameras.list(wh.id!);
    if (list.length === 0) { alert('No cameras configured for this warehouse. Add cameras first.'); return; }
    setViewCameras(list);
    setViewWarehouse(wh);
  };
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const w of warehouses) {
        if (w.id) {
          try { const list = await api.cameras.list(w.id); counts[w.id] = list.length; } catch {}
        }
      }
...
<body>
 ...
 setTimeout(() => win.print(), 500);
  ...
  {warehouses.map((w) => (
  <div key={w.id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
  <div className="p-4">
  <div className="flex items-start justify-between mb-2">
  <div>
  <h3 className="font-semibold text-gray-900">{w.name}</h3>
  <p className="text-xs text-gray-400">{w.city || 'No city'}</p>
  </div>
  <div className="flex flex-col items-end gap-1">
  <span className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${w.status === 'Active' ? 'bg-green-50 text-green-700' : 'bg-gray-50 text-gray-500'}`}>{w.status || 'Active'}</span>
  {w.code && <span className="text-[10px] font-mono font-semibold text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded">{w.code}</span>}
  </div>
  </div>
  <div className="text-xs text-gray-400 space-y-0.5 mb-3">
  {w.contact_person && <p><span className="text-gray-500">Contact:</span> {w.contact_person}</p>}
  {w.phone && <p><span className="text-gray-500">Phone:</span> {w.phone}</p>}
  {w.email && <p><span className="text-gray-500">Email:</span> {w.email}</p>}
  </div>
  <div className="grid grid-cols-3 gap-1.5">
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-gray-700">{w.total_bookings || 0}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Bookings</p>
  </div>
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-gray-700">{w.total_loadings || 0}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Loadings</p>
  </div>
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-gray-700">{w.total_receivings || 0}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Receivings</p>
  </div>
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-gray-700">{w.total_deliveries || 0}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Deliveries</p>
  </div>
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-gray-700">{(w.total_nags_received || 0).toFixed(0)}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Nags</p>
  </div>
  <div className="bg-gray-50 rounded-lg p-1.5 text-center">
  <p className="text-sm font-bold text-rose-600">{(w.total_shortage || 0).toFixed(1)}</p>
  <p className="text-[9px] text-gray-400 uppercase tracking-wider">Short</p>
  </div>
  </div>
  </div>
  <div className="flex items-center gap-0.5 px-2 py-1.5 border-t border-gray-100 bg-gray-50/50 rounded-b-xl">
  <button onClick={() => loadCameras(w)} className="flex items-center gap-1 text-[11px] text-gray-500 hover:text-indigo-600 px-2 py-1 rounded hover:bg-white transition-colors">
  <Camera size={13} /> {cameraCounts[w.id!] || 0} Cameras
  </button>
  {(cameraCounts[w.id!] || 0) > 0 && (
  <button onClick={() => openViewer(w)} className="flex items-center gap-1 text-[11px] text-gray-500 hover:text-indigo-600 px-2 py-1 rounded hover:bg-white transition-colors">
  <Monitor size={13} /> View
  </button>
  )}
  <button onClick={() => { setReportMonth(new Date().toISOString().slice(0, 7)); printMonthlyReport(w); }} className="flex items-center gap-1 text-[11px] text-gray-500 hover:text-indigo-600 px-2 py-1 rounded hover:bg-white transition-colors">
  <Printer size={13} /> Report
  </button>
  <div className="ml-auto flex gap-0.5">
  <button onClick={() => openEdit(w)} className="p-1.5 rounded hover:bg-white text-gray-400 hover:text-indigo-600 transition-colors" title="Edit">
  <Edit2 size={13} />
  </button>
  <button onClick={() => remove(w.id!)} className="p-1.5 rounded hover:bg-white text-gray-400 hover:text-red-600 transition-colors" title="Delete">
  <Trash2 size={13} />
  </button>
  </div>
  </div>
 ...
 <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="e.g., Mumbai Main Hub" />
 </div>
 <div className="col-span-2">
 <label className="block text-xs font-medium text-gray-500 mb-1">Warehouse Code *</label>
 <input value={form.code || ''} onChange={(e) => setForm({ ...form, code: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="e.g., WH-MUM-01" />
 </div>
 <div className="col-span-2">
 <label className="block text-xs font-medium text-gray-500 mb-1">Address</label>
 <textarea value={form.address || ''} onChange={(e) => setForm({ ...form, address: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" rows={2} placeholder="Full address" />
 </div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">City</label><input value={form.city || ''} onChange={(e) => setForm({ ...form, city: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 ...
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Contact Person</label><input value={form.contact_person || ''} onChange={(e) => setForm({ ...form, contact_person: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Phone</label><input value={form.phone || ''} onChange={(e) => setForm({ ...form, phone: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
   <div className="col-span-2"><label className="block text-xs font-medium text-gray-500 mb-1">Email</label><input type="email" value={form.email || ''} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
  </div>
 ...
 <button onClick={save} className="px-4 py-2 text-sm bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 font-medium">Save</button>
  ...
  {cameraManageOpen && cameraWarehouse && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => { cancelEditCam(); setCameraManageOpen(false); }}>
  <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-3 max-h-[80vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
  <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 shrink-0">
  <h3 className="text-sm font-semibold text-gray-800 flex items-center gap-2"><Camera size={16} className="text-indigo-600" /> {cameraWarehouse.name} — Cameras</h3>
  <button onClick={() => { cancelEditCam(); setCameraManageOpen(false); }} className="p-1 rounded-lg hover:bg-gray-100 text-gray-400"><X size={18} /></button>
  </div>
  <div className="p-4 overflow-y-auto flex-1">
  {cameras.length === 0 && (
  <div className="text-center py-8 text-gray-400">
  <Camera size={32} className="mx-auto mb-2 text-gray-200" />
  <p className="text-sm">No cameras yet</p>
  </div>
  )}
  <div className="space-y-1">
  {cameras.map((c) => (
  <div key={c.id} className="flex items-center justify-between py-2 px-3 rounded-lg hover:bg-gray-50 group border border-transparent hover:border-gray-100 transition-colors">
  <div className="flex items-center gap-3 min-w-0">
  <div className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" />
  <div className="min-w-0">
  <p className="text-sm font-medium text-gray-800 truncate">{c.name}</p>
  <p className="text-xs text-gray-400 font-mono truncate max-w-[320px]">{c.url}</p>
  </div>
  </div>
  <div className="flex gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
  <button onClick={() => editCam(c)} className="p-1.5 rounded hover:bg-white text-gray-400 hover:text-indigo-600"><Edit2 size={13} /></button>
  <button onClick={() => deleteCam(c.id!)} className="p-1.5 rounded hover:bg-white text-gray-400 hover:text-red-600"><Trash2 size={13} /></button>
  </div>
  </div>
  ))}
  </div>
  ...
  <p className="text-xs font-medium text-gray-500 mb-2">{editingCam ? 'Edit Camera' : 'Add Camera'}</p>
  <div className="flex flex-col gap-2">
  <div className="flex gap-2">
  <input value={camForm.name} onChange={(e) => setCamForm({ ...camForm, name: e.target.value })} placeholder="Camera name e.g. Gate 1" className="flex-1 border rounded-lg px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-500" />
  <input value={camForm.url} onChange={(e) => setCamForm({ ...camForm, url: e.target.value })} placeholder="http://192.168.1.100/video.mjpg" className="flex-[2] border rounded-lg px-3 py-2 text-sm font-mono outline-none focus:ring-2 focus:ring-indigo-500" />
  </div>
  <div className="flex gap-2">
  <button onClick={saveCam} disabled={!camForm.name || !camForm.url} className="flex-1 bg-indigo-600 text-white rounded-lg py-2 text-sm font-medium hover:bg-indigo-700 disabled:opacity-40 disabled:cursor-not-allowed transition-colors">{editingCam ? 'Update Camera' : 'Add Camera'}</button>
  {editingCam && <button onClick={cancelEditCam} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-colors">Cancel</button>}
  </div>
  </div>
  </div>
  </div>
  ...
  {viewWarehouse && viewCameras.length > 0 && (
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => { setViewWarehouse(null); setViewCameras([]); }}>
  <div className="bg-gray-900 rounded-xl overflow-hidden w-full max-w-6xl mx-3 shadow-2xl" onClick={(e) => e.stopPropagation()}>
  <div className="flex items-center justify-between px-4 py-3 bg-gray-800">
  <div className="flex items-center gap-2">
  <Monitor size={16} className="text-red-400" />
  <span className="text-sm font-medium text-white">{viewWarehouse.name} — Live ({viewCameras.length} cameras)</span>
  </div>
  <button onClick={() => { setViewWarehouse(null); setViewCameras([]); }} className="p-1 rounded-lg hover:bg-gray-700 text-gray-400"><X size={18} /></button>
  </div>
  <div className={`grid gap-1 bg-black p-1 ${viewCameras.length === 1 ? 'grid-cols-1' : viewCameras.length <= 4 ? 'grid-cols-2' : viewCameras.length <= 9 ? 'grid-cols-3' : 'grid-cols-4'}`}>
  {viewCameras.map((c) => (
  <div key={c.id} className="relative bg-black flex items-center justify-center" style={{ minHeight: '200px' }}>
  {isStreamUrl(c.url) ? (
  <img src={c.url} alt={c.name} className="w-full h-full object-contain" />
  ) : (
  <iframe src={c.url} className="w-full h-full" sandbox="allow-scripts allow-same-origin" title={c.name} />
  )}
  <span className="absolute bottom-1 left-1 bg-black/60 text-white text-[10px] px-1.5 py-0.5 rounded truncate max-w-[90%]">{c.name}</span>
  </div>
  ))}
  </div>
  </div>
