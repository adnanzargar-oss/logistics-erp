...
export default function Receivings({ tabs }: { tabs?: string[] | null }) {
 ...
 const todayReceivings = receivings.filter((r) => r.receiving_date === today || r.created_at?.startsWith(today));
 const historyReceivings = receivings.filter((r) => r.receiving_date !== today && !r.created_at?.startsWith(today));
 ...
 <h1 className="text-2xl font-bold text-gray-800 ">Load Receiving</h1>
 </div>
 </div>

 {/* New Receiving Form */}
 <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
 <h2 className="font-semibold text-gray-700 mb-4">Receive Vehicle Load</h2>

 {/* Search Load */}
 <div className="flex items-center gap-2 mb-4 pb-4 border-b ">
 <div className="flex-1">
 <label className="block text-xs font-medium text-gray-500 mb-1">Search Load Number</label>
 ...
 <div className="mb-4">
 <label className="block text-xs font-medium text-gray-500 mb-2">Search Results</label>
 <div className="border rounded-lg divide-y">
 {searchResults.map((load) => (
 <div key={load.id} className="flex items-center justify-between px-3 py-2 text-sm hover:bg-gray-50 :bg-gray-800">
 <div>
 <span className="font-mono font-medium">{load.loading_no}</span>
 <span className="text-gray-400 mx-2">|</span>
 <span className="text-gray-600 ">{load.vehicle_reg || '-'}</span>
 <span className="text-gray-400 mx-2">|</span>
 ...
 <button onClick={() => selectSearchResult(load)} className="px-3 py-1 text-xs bg-indigo-100 text-indigo-700 rounded hover:bg-indigo-200 :bg-indigo-900/50">Select</button>
 </div>
 ))}
 </div>
 </div>
 ...
 <select value={selectedVehicle} onChange={(e) => handleVehicleChange(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 ...
 <select value={selectedWarehouse} onChange={(e) => setSelectedWarehouse(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 <option value="">Select warehouse</option>
 {warehouses.map((w) => <option key={w.id} value={w.id!}>{w.code ? `${w.code} - ` : ''}{w.name}</option>)}
 </select>
 </div>
 <div>
 <label className="block text-xs font-medium text-gray-500 mb-1">Receiving Date</label>
 <input type="date" value={receivingDate} onChange={(e) => setReceivingDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
 </div>
 ...
 <p className="text-sm text-gray-400 py-4 text-center border rounded-lg bg-gray-50 ">No loaded LRs found for this vehicle. Load the vehicle first.</p>
 ) : (
 <div>
 {/* Select which load to receive */}
 {vehicleLoads.length > 1 && (
 <div className="mb-3">
 <label className="block text-xs font-medium text-gray-500 mb-1">Select Load Trip</label>
 <div className="flex gap-2">
 ...
 selectedLoad === load ? 'bg-indigo-50 border-indigo-400 text-indigo-700' : 'hover:bg-gray-50 :bg-gray-800 '
 }`}
 >
 ...
 {/* LRs Table */}
 <label className="block text-xs font-medium text-gray-500 mb-2">Enter Received Nags for each LR (expected nags shown)</label>
 ...
 return (
 <tr key={b.booking_id} className={`hover:bg-gray-50 :bg-gray-800 text-xs ${short > 0 ? 'bg-red-50 ' : ''}`}>
 ...
 className="w-20 border rounded px-2 py-1 text-xs text-right focus:ring-2 focus:ring-indigo-500 outline-none"
 min={0}
 />
 </td>
 ...
 className="w-full border rounded px-2 py-1 text-xs focus:ring-2 focus:ring-indigo-500 outline-none"
 />
 ) : '-'}
 </td>
 </tr>
 ...
 <td></td>
  ...
  {allowedTabs.includes('today') && (
    <button onClick={() => setTab('today')}
      ...
      Today {todayReceivings.length > 0 && <span className="ml-1 text-xs text-gray-400">({todayReceivings.length})</span>}
    </button>
  )}
  ...
  {(tab === 'today' ? todayReceivings : historyReceivings).map((r) => (
 ...
 <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-medium">{r.status}</span>
 </td>
 <td className="py-3 px-4">
 <div className="flex gap-1">
 <button onClick={() => { api.receivings.get(r.id!).then(setDetail); }} className="p-1 hover:bg-gray-100 :bg-gray-800 rounded" title="View"><Eye size={14} className="text-gray-400 " /></button>
 ...
 <button onClick={() => remove(r.id!)} className="p-1 hover:bg-red-50 :bg-red-900/20 rounded" title="Delete"><Trash2 size={14} className="text-red-400" /></button>
 </div>
 </td>
 </tr>
 ))}
  {(tab === 'today' ? todayReceivings : historyReceivings).length === 0 && <tr><td colSpan={8} className="text-center py-10 text-gray-400 ">{tab === 'today' ? 'No receivings today' : 'No receivings yet'}</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 <div><span className="text-gray-400 ">Date:</span> {detail.receiving_date ? new Date(detail.receiving_date).toLocaleDateString() : '-'}</div>
 <div><span className="text-gray-400 ">Warehouse:</span> {detail.warehouse_name || '-'}</div>
 </div>
 <div className="border rounded-lg">
 <div className="overflow-x-auto">
 <table className="w-full text-sm">
 ...
 const short = b.short_bags ?? 0;
 return (
 <tr key={b.id} className={`hover:bg-gray-50 :bg-gray-800 text-xs ${short > 0 ? 'bg-red-50 ' : ''}`}>
 <td className="py-2 px-3 font-mono font-medium">{b.booking_no}</td>
 <td className="py-2 px-3 max-w-[120px] truncate">{b.consignor_name || '-'}</td>
 <td className="py-2 px-3 max-w-[120px] truncate">{b.consignee_name || '-'}</td>
 <td className="py-2 px-3 text-right">{b.num_bags ?? '-'}</td>
