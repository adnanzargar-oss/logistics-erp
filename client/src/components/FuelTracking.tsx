...
export default function FuelTracking() {
 ...
 <h1 className="text-2xl font-bold text-gray-800 ">Fuel Tracking</h1>
 </div>
 ...
 <div className="bg-white rounded-xl p-4 shadow-sm border text-center">
 <p className="text-xs text-gray-500 ">Avg Cost/Ltr</p>
 ...
 <td className="py-3 px-4 text-xs text-gray-500 ">{e.odometer_km ? `${e.odometer_km} km` : '-'}</td>
 <td className="py-3 px-4 text-xs">{e.station_name || '-'}</td>
 <td className="py-3 px-4 text-xs">{e.paid_by}</td>
 <td className="py-3 px-4">
 <button onClick={() => remove(e.id!)} className="p-1 hover:bg-red-50 :bg-red-900/20 rounded"><Trash2 size={14} className="text-red-400" /></button>
 </td>
 </tr>
 ))}
 {entries.length === 0 && <tr><td colSpan={9} className="text-center py-10 text-gray-400 ">No fuel entries yet</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Date *</label><input type="date" value={form.fuel_date} onChange={(e) => setForm({ ...form, fuel_date: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Quantity (Ltr) *</label><input type="number" step="0.01" value={form.quantity_ltr || ''} onChange={(e) => { const q = Number(e.target.value); setForm({ ...form, quantity_ltr: q, total_cost: calcTotal(q, form.cost_per_ltr) }); }} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Cost/Ltr *</label><input type="number" step="0.01" value={form.cost_per_ltr || ''} onChange={(e) => { const c = Number(e.target.value); setForm({ ...form, cost_per_ltr: c, total_cost: calcTotal(form.quantity_ltr, c) }); }} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 ...
 <div className="col-span-2"><label className="block text-xs font-medium text-gray-500 mb-1">Station Name</label><input value={form.station_name || ''} onChange={(e) => setForm({ ...form, station_name: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" /></div>
 ...
 <div><label className="block text-xs font-medium text-gray-500 mb-1">Paid By</label>
 <select value={form.paid_by} onChange={(e) => setForm({ ...form, paid_by: e.target.value })} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
