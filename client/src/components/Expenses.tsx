  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3 mb-6">
        {categories.map((c: any) => (
          <button key={c.expense_category} onClick={() => setFilterCat(filterCat === c.expense_category ? '' : c.expense_category)}
            className={`text-center rounded-xl p-3 border text-sm transition-all ${filterCat === c.expense_category ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-300 text-indigo-700' : 'bg-white dark:bg-gray-900 border-gray-100 dark:border-gray-700 hover:border-gray-200'}`}>
      ...
      <div className="grid grid-cols-4 gap-3 mb-6">
        <div className="bg-white dark:bg-gray-900 rounded-lg px-4 py-3 border border-gray-100 dark:border-gray-700"><span className="text-[11px] text-gray-400">Total</span><p className="text-lg font-bold text-gray-800 dark:text-gray-100">{summary.total.toLocaleString()}</p></div>
        <div className="bg-white dark:bg-gray-900 rounded-lg px-4 py-3 border border-gray-100 dark:border-gray-700"><span className="text-[11px] text-gray-400">Paid</span><p className="text-lg font-bold text-green-600">{summary.paid.toLocaleString()}</p></div>
        <div className="bg-white dark:bg-gray-900 rounded-lg px-4 py-3 border border-gray-100 dark:border-gray-700"><span className="text-[11px] text-gray-400">Unpaid</span><p className="text-lg font-bold text-red-600">{summary.unpaid.toLocaleString()}</p></div>
        <div className="bg-white dark:bg-gray-900 rounded-lg px-4 py-3 border border-gray-100 dark:border-gray-700"><span className="text-[11px] text-gray-400">Entries</span><p className="text-lg font-bold text-gray-800 dark:text-gray-100">{summary.count}</p></div>
      </div>
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800">
              {expenses.map((e: any) => (
                <tr key={e.id} className="hover:bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-900">
                  <td className="py-3 px-4 font-mono text-xs font-medium text-gray-800 dark:text-gray-200">{e.voucher_no || `#${e.id}`}</td>
                  <td className="py-3 px-4 text-xs">{new Date(e.expense_date).toLocaleDateString()}</td>
                  <td className="py-3 px-4"><span className="bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded text-xs">{e.expense_type}</span></td>
                  <td className="py-3 px-4 text-xs">{e.expense_category}</td>
                  <td className="py-3 px-4 text-xs text-gray-600 dark:text-gray-300 truncate max-w-[120px]">{e.vendor_name || '-'}</td>
                  ...
                  <td className="py-3 px-4">
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                      e.status === 'Paid' || e.status === 'Approved' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                      e.status === 'Pending Approval' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' :
                      e.status === 'Cancelled' || e.status === 'Rejected' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-1 justify-end">
                      <button onClick={() => printPaymentVoucher(e)} className="p-1.5 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 rounded" title="Print Payment Voucher"><Printer size={13} className="text-indigo-400" /></button>
                      <button onClick={() => openEdit(e)} className="p-1.5 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded" title="Edit"><Pencil size={13} className="text-blue-400" /></button>
                      <button onClick={() => remove(e.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded"><Trash2 size={13} className="text-red-400" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editId ? 'Edit Expense' : 'Add Expense'}>
        {/* Tabs */}
        <div className="flex gap-1 mb-4 border-b border-gray-200 dark:border-gray-700 -mx-4 px-4">
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-xs font-medium text-gray-500 mb-1">Voucher No</label>
                  <input value={form.voucher_no || ''} readOnly className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 text-gray-500" /></div>
                <div><label className="block text-xs font-medium text-gray-500 mb-1">Voucher Date *</label>
                  <input type="date" value={form.expense_date} onChange={(e) => update({ expense_date: e.target.value })}
                    className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-100" /></div>
                <div><label className="block text-xs font-medium text-gray-500 mb-1">Posting Date</label>
                  <input type="date" value={form.posting_date || ''} onChange={(e) => update({ posting_date: e.target.value })}
                    className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-gray-700" /></div>
                <div><label className="block text-xs font-medium text-gray-500 mb-1">Expense Type *</label>
                  <select value={form.expense_type} onChange={(e) => update({ expense_type: e.target.value })}
                    className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-gray-700">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2"><label className="block text-xs font-medium text-gray-500 mb-1">Address</label>
                <textarea value={form.vendor_address || ''} onChange={(e) => update({ vendor_address: e.target.value })}
                  className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-gray-700" rows={2} /></div>
          ...
          {tab === 'details' && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="block text-xs font-medium text-gray-500 mb-1">Tax Amount</label>
                <input value={(form.tax_amount || 0).toFixed(2)} readOnly className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-gray-50 dark:bg-gray-800 text-gray-500" /></div>
          ...
          {tab === 'notes' && (
            <div>
              ...
              <textarea value={form.notes || ''} onChange={(e) => update({ notes: e.target.value })}
                className="w-full border dark:border-gray-600 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none bg-white dark:bg-gray-700" rows={6}
        ...
        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
          <button onClick={() => setModal(false)} className="px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg">Cancel</button>
