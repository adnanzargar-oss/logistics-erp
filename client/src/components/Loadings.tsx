...
import { api } from '../api';
import { Booking, Vehicle, Driver, Loading, Warehouse } from '../types';
import { PackageCheck, Plus, Trash2, Printer, Eye, Warehouse as WarehouseIcon } from 'lucide-react';
...
export default function Loadings({ tabs, actions }: { tabs?: string[] | null; actions?: string[] | null }) {
  ...
  const [tab, setTab] = useState<'new' | 'transit' | 'history'>(defaultTab);
 ...
 prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]
  ...
  setReviewOpen(true);
   ...
   from_warehouse_id: fromWarehouse || undefined,
   ...
   alert('Failed to create loading: ' + (e as Error).message);
   }
   };
 ...
 <h1 className="text-2xl font-bold text-gray-800 ">Loading</h1>
  ...
  {allowedTabs.includes('new') && (
    <button onClick={() => setTab('new')} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${tab === 'new' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700 :text-gray-300'}`}>New Vehicle Loading</button>
  )}
  {allowedTabs.includes('transit') && (
    <button onClick={() => setTab('transit')} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${tab === 'transit' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700 :text-gray-300'}`}>In Transit ({transitLoadings.length})</button>
  )}
  {allowedTabs.includes('history') && (
    <button onClick={() => setTab('history')} className={`px-4 py-1.5 text-sm font-medium rounded-md transition-colors ${tab === 'history' ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700 :text-gray-300'}`}>Loading History ({historyLoadings.length})</button>
  )}
  </div>
 ...
 <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-5 mb-6">
  <h2 className="font-semibold text-gray-700 mb-4">New Vehicle Loading</h2>
  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
  <div>
  <label className="block text-xs font-medium text-gray-500 mb-1">Vehicle *</label>
  <select value={selectedVehicle} onChange={(e) => setSelectedVehicle(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
  ...
  <select value={fromWarehouse} onChange={(e) => setFromWarehouse(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
  <option value="">Select warehouse</option>
  {warehouses.map((w) => <option key={w.id} value={w.id!}>{w.name}</option>)}
  </select>
  </div>
  <div>
  <label className="block text-xs font-medium text-gray-500 mb-1">To Warehouse (Unloading)</label>
  <select value={toWarehouse} onChange={(e) => setToWarehouse(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
  <option value="">Select warehouse</option>
  {warehouses.map((w) => <option key={w.id} value={w.id!}>{w.name}</option>)}
  </select>
  </div>
  <div>
  <label className="block text-xs font-medium text-gray-500 mb-1">Driver (auto-assigned)</label>
  <select value={selectedDriver} onChange={(e) => setSelectedDriver(Number(e.target.value) || '')} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
  <option value="">Select driver</option>
  {drivers.map((d) => <option key={d.id} value={d.id!}>{d.name} - {d.license_number || 'No license'}</option>)}
  </select>
  </div>
  <div>
  <label className="block text-xs font-medium text-gray-500 mb-1">Loading Date</label>
  <input type="date" value={loadingDate} onChange={(e) => setLoadingDate(e.target.value)} className="w-full border rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
  </div>
 ...
 <label className="block text-xs font-medium text-gray-500 ">Select LR(s) to Load ({selectedLRs.length} selected)</label>
 ...
 <p className="text-sm text-gray-400 py-4 text-center border rounded-lg bg-gray-50 ">No unloaded LRs available. Create a booking first.</p>
 ) : (
 ...
 selected ? 'bg-indigo-50 ' : 'hover:bg-gray-50 :bg-gray-800/50 '
 }`}
 >
 ...
 <td className="py-2 px-3 text-gray-400 ">{b.from_location || '-'} → {b.to_location || '-'}</td>
 <td className={`py-2 px-3 font-mono ${b.eway_bill_no ? 'text-amber-600 font-semibold' : 'text-gray-300 '}`}>{b.eway_bill_no || '-'}</td>
 </tr>
 );
 })}
 </tbody>
 ...
 {tab === 'transit' && (
 <>
 <h2 className="font-semibold text-gray-700 mb-3">In Transit ({transitLoadings.length})</h2>
 ...
 <tr key={l.id} className="hover:bg-gray-50 :bg-gray-800 ">
 ...
 <span className="bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full text-xs font-medium">In Transit</span>
 </td>
 <td className="py-3 px-4">
 <div className="flex gap-1">
 <button onClick={() => { api.loadings.get(l.id!).then(setDetail); }} className="p-1 hover:bg-gray-100 :bg-gray-800 rounded" title="View"><Eye size={14} className="text-gray-400 " /></button>
 <button onClick={() => { api.loadings.get(l.id!).then((d) => printLoadSheet(d)); }} className="p-1 hover:bg-blue-50 rounded" title="Print Load Sheet"><Printer size={14} className="text-blue-400" /></button>
  {canDelete && <button onClick={() => remove(l.id!)} className="p-1 hover:bg-red-50 :bg-red-900/20 rounded" title="Unload"><Trash2 size={14} className="text-red-400" /></button>}
  </div>
  </td>
  </tr>
  ))}
  {transitLoadings.length === 0 && <tr><td colSpan={8} className="text-center py-10 text-gray-400 ">No loads in transit</td></tr>}
 </tbody>
 </table>
 </div>
 </div>
 ...
 {tab === 'history' && (
 <>
 <h2 className="font-semibold text-gray-700 mb-3">Loading History</h2>
 <div className="flex flex-wrap items-center gap-2 mb-3">
 <div className="flex gap-1 bg-gray-100 rounded-lg p-0.5">
 {(['all', 'vehicle', 'driver', 'date'] as const).map((m) => (
 <button key={m} onClick={() => { setFilterMode(m); setFilterVehicle(''); setFilterDriver(''); setFilterDate(''); }} className={`px-3 py-1 text-xs font-medium rounded-md transition-colors ${filterMode === m ? 'bg-white text-gray-800 shadow-sm' : 'text-gray-500 hover:text-gray-700 :text-gray-300'}`}>
 {m === 'all' ? 'All' : m === 'vehicle' ? 'By Vehicle' : m === 'driver' ? 'By Driver' : 'By Date'}
 </button>
 ))}
 </div>
 {filterMode === 'vehicle' && (
 <select value={filterVehicle} onChange={(e) => setFilterVehicle(e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 <option value="">All Vehicles</option>
 {[...new Set(historyLoadings.map((l) => l.vehicle_reg).filter(Boolean))].map((v) => <option key={v} value={v!}>{v}</option>)}
 </select>
 )}
 {filterMode === 'driver' && (
 <select value={filterDriver} onChange={(e) => setFilterDriver(e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
 <option value="">All Drivers</option>
 {[...new Set(historyLoadings.map((l) => l.driver_name).filter(Boolean))].map((d) => <option key={d} value={d!}>{d}</option>)}
 </select>
 )}
 {filterMode === 'date' && (
 <input type="date" value={filterDate} onChange={(e) => setFilterDate(e.target.value)} className="border rounded-lg px-3 py-1.5 text-sm focus:ring-2 focus:ring-indigo-500 outline-none" />
 )}
 </div>
 <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
 <div className="overflow-x-auto">
 ...
 <td className="py-3 px-4 text-xs">{l.item_count ?? 0}</td>
 <td className="py-3 px-4">
 <span className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-xs font-medium">{l.status}</span>
 </td>
 <td className="py-3 px-4">
 <div className="flex gap-1">
 <button onClick={() => { api.loadings.get(l.id!).then(setDetail); }} className="p-1 hover:bg-gray-100 :bg-gray-800 rounded" title="View"><Eye size={14} className="text-gray-400 " /></button>
 ...
 {filteredHistoryLoadings.length === 0 && <tr><td colSpan={8} className="text-center py-10 text-gray-400 ">{filterMode !== 'all' ? 'No loads match your filter' : 'No loads yet'}</td></tr>}
 </tbody>
 </table>
 </div>
  </div>
  ...
  <div className="space-y-4">
   ...
   <div><span className="text-gray-400 ">Vehicle:</span> <span className="font-medium">{vehicles.find((v) => v.id === selectedVehicle)?.reg_number || '-'}</span></div>
   <div><span className="text-gray-400 ">From:</span> <span className="font-medium">{warehouses.find((w) => w.id === fromWarehouse)?.name || '-'}</span></div>
   <div><span className="text-gray-400 ">To:</span> <span className="font-medium">{warehouses.find((w) => w.id === toWarehouse)?.name || '-'}</span></div>
   <div><span className="text-gray-400 ">Driver:</span> {drivers.find((d) => d.id === selectedDriver)?.name || '-'}</div>
   <div><span className="text-gray-400 ">Date:</span> {loadingDate ? new Date(loadingDate).toLocaleDateString() : '-'}</div>
   <div><span className="text-gray-400 ">LRs:</span> {selectedLRs.length}</div>
   </div>
  <div className="border rounded-lg">
  <div className="overflow-x-auto">
  <table className="w-full text-sm">
  ...
  <button onClick={() => setReviewOpen(false)} className="px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 :bg-gray-800 rounded-lg">Back</button>
  ...
  }} className="flex items-center gap-1.5 px-4 py-2 text-sm border border-indigo-600 text-indigo-600 rounded-lg hover:bg-indigo-50 :bg-indigo-900/20"><Printer size={16} /> Print</button>
 ...
 <div className="space-y-4">
  <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-sm">
  <div><span className="text-gray-400 ">Vehicle:</span> <span className="font-medium">{detail.vehicle_reg}</span></div>
  <div><span className="text-gray-400 ">From:</span> {detail.from_warehouse_name || '-'}</div>
  <div><span className="text-gray-400 ">To:</span> {detail.to_warehouse_name || '-'}</div>
  <div><span className="text-gray-400 ">Driver:</span> {detail.driver_name || '-'}</div>
  <div><span className="text-gray-400 ">License:</span> {detail.license_number || '-'}</div>
  <div><span className="text-gray-400 ">Date:</span> {detail.loading_date ? new Date(detail.loading_date).toLocaleDateString() : '-'}</div>
  </div>
 <div className="border rounded-lg">
 <div className="overflow-x-auto">
 <table className="w-full text-sm">
